...
write.csv2(table_data, file = "Results/CoefficientsModel1v3.csv", row.names = FALSE)
```
...
# Save coefficients as csv
table_data <- tibble::rownames_to_column(as.data.frame(results_vert$estimates[, c("Estimate", "Std.Error", "P(>|t|)")]), var = "Predictor")
write.csv2(table_data, file = "Results/CoefficientsModel1bv3.csv", row.names = FALSE)
```
...
fit <- imp_all %>% 
  map(~.x %$% lme4::lmer(
    q60_1 ~ (1 | id) + time_yr + I(time_yr^2) +
      age_cat + sekse + etn_herkomst + edu + huis, 
    weights = .x$weegfactor
  )) 
```
...
# Save coefficients as csv
table_data <- tibble::rownames_to_column(as.data.frame(results_vert$estimates[, c("Estimate", "Std.Error", "P(>|t|)")]), var = "Predictor")
write.csv2(table_data, file = "Results/CoefficientsModel2v3.csv", row.names = FALSE)
```
...
# Save coefficients as csv
table_data <- tibble::rownames_to_column(as.data.frame(results_vert$estimates[, c("Estimate", "Std.Error", "P(>|t|)")]), var = "Predictor")
write.csv2(table_data, file = "Results/CoefficientsModel3v3.csv", row.names = FALSE)
```
  ...
  map(
    ~ .x %$% lme4::lmer(
      q60_1 ~ (1 | id) + time_yr + 
        age_cat + sekse + etn_herkomst + edu + huis +
        gezo + rond + drei + onvr +
        time_yr:age_cat + time_yr:sekse + time_yr:etn_herkomst + time_yr:edu + time_yr:huis,
      weights = .x$weegfactor
    )
  )
```
...
# Save coefficients as csv
table_data <- tibble::rownames_to_column(as.data.frame(results_vert$estimates[, c("Estimate", "Std.Error", "P(>|t|)")]), var = "Predictor")
write.csv2(table_data, file = "Results/CoefficientsModel4v3.csv", row.names = FALSE)
```
...
write.csv2(table_data, file = "Results/CoefficientsModel5v3.csv", row.names = FALSE)
```
