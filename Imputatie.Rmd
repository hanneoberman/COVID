...
modus <- function(x) {
  ...
  if(dim(tally)==0) {mode <- NA} else {mode <- which.max(tally)}
  return(mode)
}
...
dat_clean %<>% 
  group_by(id) %>% 
  mutate(across(all_of(vars_fixed), ~ modus(.x))) %>% 
  ungroup()

### CHECK IF INCONSISTENCIES ARE GONE and if missings are preserved?
dat_clean[1:200,] %>% 
...
modi <- aggregate(dat_clean[, c("id", vars_fixed)], by = list(id = dat_clean$id), FUN = modus)
